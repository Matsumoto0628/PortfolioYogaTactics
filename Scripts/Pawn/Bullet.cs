...
public class Bullet : MonoBehaviour
{
    ...
    public void Initialize(bool isEnemy, int hitFPS, int attack, float hitStrength)
    {
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        if (isEnemy) // 敵は左向きにUnitとPlayerを検知
        {
            if (collision.gameObject.CompareTag("Unit"))
            {
                ...
                collision.gameObject.GetComponent<TD_Unit>().PawnController.Hit(1f, 0.05f);
        ...
        else // 味方は右向きにEnemyとEnemyCastleを検知
        {
            if (collision.gameObject.CompareTag("Enemy"))
            {
                ...
                collision.gameObject.GetComponent<TD_Enemy>().PawnController.Hit(1f, 0.05f);
